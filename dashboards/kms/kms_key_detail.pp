dashboard "kms_key_detail" {
  ...
  with "bigquery_datasets_for_kms_key" {
    query = query.bigquery_datasets_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "bigquery_tables_for_kms_key" {
    query = query.bigquery_tables_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "compute_disks_for_kms_key" {
    query = query.compute_disks_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "compute_images_for_kms_key" {
    query = query.compute_images_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "compute_snapshots_for_kms_key" {
    query = query.compute_snapshots_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "kms_key_rings_for_kms_key" {
    query = query.kms_key_rings_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "kubernetes_clusters_for_kms_key" {
    query = query.kubernetes_clusters_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "pubsub_topics_for_kms_key" {
    query = query.pubsub_topics_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "sql_backups_for_kms_key" {
    query = query.sql_backups_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "sql_database_instances_for_kms_key" {
    query = query.sql_database_instances_for_kms_key
    args  = [self.input.key_self_link.value]
  }

  with "storage_buckets_for_kms_key" {
    query = query.storage_buckets_for_kms_key
    args  = [self.input.key_self_link.value]
  }
  ...
  container {
    ...
    graph {
      ...
      node {
        ...
        args = {
          bigquery_dataset_ids = with.bigquery_datasets_for_kms_key.rows[*].dataset_id
        }
      }
      ...
      node {
        ...
        args = {
          bigquery_table_ids = with.bigquery_tables_for_kms_key.rows[*].table_id
        }
      }
      ...
      node {
        ...
        args = {
          compute_disk_ids = with.compute_disks_for_kms_key.rows[*].disk_id
        }
      }
      ...
      node {
        ...
        args = {
          compute_image_ids = with.compute_images_for_kms_key.rows[*].image_id
        }
      }
      ...
      node {
        ...
        args = {
          compute_snapshot_names = with.compute_snapshots_for_kms_key.rows[*].snapshot_name
        }
      }
      ...
      node {
        ...
        args = {
          kms_key_ring_names = with.kms_key_rings_for_kms_key.rows[*].akas
        }
      }
      ...
      node {
        ...
        args = {
          kubernetes_cluster_ids = with.kubernetes_clusters_for_kms_key.rows[*].cluster_id
        }
      }
      ...
      node {
        ...
        args = {
          pubsub_topic_self_links = with.pubsub_topics_for_kms_key.rows[*].self_link
        }
      }
      ...
      node {
        ...
        args = {
          sql_backup_ids = with.sql_backups_for_kms_key.rows[*].backup_id
        }
      }
      ...
      node {
        ...
        args = {
          database_instance_self_links = with.sql_database_instances_for_kms_key.rows[*].self_link
        }
      }
...
query "bigquery_datasets_for_kms_key" {
  sql = <<-EOQ
    select
      d.id as dataset_id
    from
...
query "compute_snapshots_for_kms_key" {
  sql = <<-EOQ
    select
...
query "kms_key_rings_for_kms_key" {
  sql = <<-EOQ
    select
      p.akas::text
    from
...
query "kubernetes_clusters_for_kms_key" {
  sql = <<-EOQ
    select
...
query "pubsub_topics_for_kms_key" {
  sql = <<-EOQ
    select
      p.self_link
    from
...
query "sql_database_instances_for_kms_key" {
  sql = <<-EOQ
    select
      i.self_link
    from
