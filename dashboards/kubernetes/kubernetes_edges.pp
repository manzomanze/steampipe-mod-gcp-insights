...
edge "kubernetes_cluster_to_compute_subnetwork" {
  ...
  sql = <<-EOQ
    ...
    from
      gcp_all.gcp_kubernetes_cluster c
      join unnest($1::text[]) as u on c.id = split_part(u, '/', 1) and c.project = split_part(u, '/', 2)
      join gcp_all.gcp_compute_network n on c.network = n.name
      left join gcp_all.gcp_compute_firewall f on n.self_link = f.network
      left join gcp_all.gcp_compute_subnetwork s on s.self_link like '%' || (c.network_config ->> 'Subnetwork') || '%';
  EOQ
